let Reporter = require('../base').Reporter;
let EncoderBuffer = require('../base').EncoderBuffer;
//let assert = require('double-check').assert;
...
Node.prototype._useArgs = function useArgs(args) {
  ...
  let children = args.filter(function(arg) {
...
Node.prototype._decode = function decode(input) {
  ...
  if (present) {
    ...
    if (!state.any && state.choice === null && state.children !== null) {
      let fail = state.children.some(function decodeChildren(child) {
...
Node.prototype._decodeChoice = function decodeChoice(input) {
  ...
  Object.keys(state.choice).some(function(key) {
    let save = input.save();
    let node = state.choice[key];
    try {
      let value = node._decode(input);
...
Node.prototype._encodeValue = function encode(data, reporter, parent) {
  ...
  if (!state.any && state.choice === null) {
    ...
    let cls = state.implicit === null ? 'universal' : 'context';
...
Node.prototype._encodeChoice = function encodeChoice(data, reporter) {
  ...
  let node = state.choice[data.type];
  // if (!node) {
  //   assert(
  //       false,
